...
class AdminUserController extends BaseProjectAdminController {
	...
	async insertUser() {
		...
		this.logUser('添加了待注册用户「' + input.name + '(' + input.mobile + ')」');
	...
	async delUser() {
		...
		let mobile = await UserModel.getOneField({ USER_MINI_OPENID: input.id }, 'USER_MOBILE');

		let service = new AdminUserService();
		await service.delUser(input.id);

		if (title)
			this.logUser('删除了用户「' + title + '(' + mobile + ')」');
